...
class YOLOV3(object):
    ...
    def __build_nework(self, input_data):
        ...
        route_1, route_2, input_data = backbone.peleetnet_yolov3(input_data, self.trainable)

        input_data = common.convolutional(input_data, (1, 1, 1024, 512), self.trainable, 'conv52')
        # input_data = common.convolutional(input_data, (3, 3,  512, 1024), self.trainable, 'conv53')
        input_data = common.separable_conv('conv53', input_data, 512, 1024, self.trainable)
        input_data = common.RFB_net(input_data, self.trainable, 'RFB_net', stride=1, scale=0.1)
        input_data = common.convolutional(input_data, (1, 1, 1024, 512), self.trainable, 'conv54')
        # input_data = common.convolutional(input_data, (3, 3,  512, 1024), self.trainable, 'conv55')
        input_data = common.separable_conv('conv55', input_data, 512, 1024, self.trainable)
        ...
        input_data = common.separable_conv('conv59', input_data, 256, 512, self.trainable)
        # input_data = tf.nn.depthwise_conv2d(input_data, (3, 3, 256, 2), strides=[1, 1, 1, 1], padding="SAME",name='conv59')
        input_data = common.convolutional(input_data, (1, 1, 512, 256), self.trainable, 'conv60')
        # input_data = common.convolutional(input_data, (3, 3, 256, 512), self.trainable, 'conv61')
        input_data = common.separable_conv('conv61', input_data, 256, 512, self.trainable)
        ...
        input_data = common.separable_conv('conv65', input_data, 128, 256, self.trainable)

        input_data = common.convolutional(input_data, (1, 1, 256, 128), self.trainable, 'conv66')
        # input_data = common.convolutional(input_data, (3, 3, 128, 256), self.trainable, 'conv67')
        input_data = common.separable_conv('conv67', input_data, 128, 256, self.trainable)
    ...
    def bbox_ciou(self, boxes1, boxes2):
        ...
        ciou = iou - 1.0 * p2 / tf.maximum(enclose_c2, 1e-12) - 1.0 * a * v
        return ciou
    ...
    def loss_layer(self, conv, pred, label, bboxes, anchors, stride):
        ...
        giou = tf.expand_dims(self.bbox_ciou(pred_xywh, label_xywh), axis=-1)
    ...
    def compute_loss(self, label_sbbox, label_mbbox, label_lbbox, true_sbbox, true_mbbox, true_lbbox):
        ...
        loss_bbox_s = loss_sbbox[0] + loss_sbbox[1] + loss_sbbox[2]
        loss_bbox_m = loss_mbbox[0] + loss_mbbox[1] + loss_mbbox[2]
        loss_bbox_l = loss_lbbox[0] + loss_lbbox[1] + loss_lbbox[2]

        return giou_loss, conf_loss, prob_loss, loss_bbox_s, loss_bbox_m, loss_bbox_l
