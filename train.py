...
class YoloTrain(object):
    def __init__(self):  # 从config文件获取到一些变量
        ...
        self.train_logdir = "/content/drive/MyDrive/Voc/checkpoint/log/train"  # 日志保存地址
        self.trainset = Dataset('train')
        self.testset = Dataset('test')
        self.steps_per_period = len(self.trainset)
        self.sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))
        self.train_batchsize = cfg.TRAIN.BATCH_SIZE
        ...
        with tf.name_scope("define_loss"):  # 定义损失函数
            ...
            self.giou_loss, self.conf_loss, self.prob_loss, self.loss_bbox_s, self.loss_bbox_m, self.loss_bbox_l = self.model.compute_loss(
        ...
        with tf.name_scope("define_first_stage_train"):  # 第一阶段训练，只训练指定层
            ...
            first_stage_optimizer = tf.train.AdamOptimizer(self.learn_rate).minimize(self.loss + self.l2_loss_1,
        ...
        with tf.name_scope("define_second_stage_train"):  # 第二阶段训练，释放所有层
            ...
            second_stage_optimizer = tf.train.AdamOptimizer(self.learn_rate).minimize(self.loss + self.l2_loss_1,
        ...
        with tf.name_scope('summary'):
            ...
            tf.summary.scalar('train_batch_statistics/loss_l2_1', self.l2_loss_1)
            tf.summary.scalar("learn_rate", self.learn_rate)
            tf.summary.scalar("giou_loss", self.giou_loss)
            tf.summary.scalar("conf_loss", self.conf_loss)
            tf.summary.scalar("prob_loss", self.prob_loss)
            tf.summary.scalar("self.loss_bbox_s", self.loss_bbox_s)
            tf.summary.scalar("self.loss_bbox_m", self.loss_bbox_m)
            tf.summary.scalar("self.loss_bbox_l", self.loss_bbox_l)
            tf.summary.scalar("total_loss", self.loss)

            logdir = "/content/drive/MyDrive/Voc/log/"  # 日志保存地址
    ...
    def train(self):
        ...
        for epoch in range(0, 1 + self.first_stage_epochs + self.second_stage_epochs):
            ...
            for train_data in pbar:
                _, summary, train_step_loss, global_step_val, lr, l2_loss_1 = self.sess.run(
                    [train_op, self.write_op, self.loss, self.global_step, self.learn_rate, self.l2_loss_1], feed_dict={
            ...
            self.summary_writer.add_summary(make_summary('evaluation/test_step_loss', test_epoch_loss),
                                            global_step=epoch)
            ckpt_file = "/content/drive/MyDrive/Voc/checkpoint/yolov3_train_loss=%.4f.ckpt" % train_epoch_loss
            ...
            if test_epoch_loss < test_loss:
                ...
                ckpt_file = "/content/drive/MyDrive/Voc/checkpoint/best_loss/yolov3_best_loss=%.4f.ckpt" % train_epoch_loss
