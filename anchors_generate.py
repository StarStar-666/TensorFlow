...
#
#
# saveBasePath = r"D:/yymnist-master/voc_2007/JPEGImages"              # txt文件保存目录
# total_xml = os.listdir(r'D:/yymnist-master/voc_2007/Annotations')   # 获取标注文件（file_name.xml）
#
...
def parse_anno(annotation_path, target_size=None):
    ...
    for line in anno:
        ...
        for i in range(box_cnt):
            ...
            x_min, y_min, x_max, y_max = float((s[i]).split(",")[0]), float((s[i]).split(",")[1]), float((s[i]).split(",")[2]), float((s[i].split(",")[3]))
...
if __name__ == '__main__':
    ...
    target_size = [416,416]            #   1,3, 4,4, 3,8, 9,6, 7,13, 17,10, 13,20, 27,23, 50,46
    annotation_path = "D:/tensorflow-yolov3/tensorflow-serving-yolov3-master-peleenet/tensorflow-serving-yolov3-master/data/dataset/1.txt"
